...
function parseArgs(argv) {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (arg === "--timeout") {
			...
			if (Number.isFinite(parsed)) out.timeoutMs = Math.max(1000, parsed);
			continue;
		}
...
function usage() {
	return `Usage:
  node search.mjs "<query>" [--purpose "<why>"] [--provider openai-codex|anthropic] [--model <id>] [--timeout <ms>] [--json]
...
function resolveAnthropicUrl(baseUrl = "https://api.anthropic.com") {
...
async function runAnthropicSearch({ model, apiKey, query, purpose, timeoutMs, baseUrl }) {
	const body = {
		model,
		max_tokens: 1800,
		temperature: 0,
	...
	const endpoint = resolveAnthropicUrl(baseUrl);
